...
final class SystemLogger {
   ...
   */
  private void println(@Nonnull final PrintStream stream, @Nonnull final String level, @Nullable final String
          message, @Nullable final Throwable th) {
    if (Options.check("verbose")) {
      final String formatted;
      try (Formatter formatter = new Formatter()) {
        formatted = formatter.format("%1$tY-%1$tm-%1$td %1$tH:%1$tM:%1$tS,%1$tL [%2$5S] %3$s: %4$s", new Date(),
                level, name, message).toString();
      }
